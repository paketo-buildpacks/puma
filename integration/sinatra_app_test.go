...
func testSinatraApp(t *testing.T, context spec.G, it spec.S) {
	var (
	...
	context("when building a sinatra app", func() {
		var (
			image     occam.Image
		...
		it("creates a working OCI image with a puma start command", func() {
			var err error
			source, err = occam.Source(filepath.Join("testdata", "sinatra_app"))
			Expect(err).NotTo(HaveOccurred())
